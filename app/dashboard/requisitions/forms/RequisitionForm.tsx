...
export default function NewRequisitionForm({
  ...
  useEffect(() => {
    if (requisition) {
      const loadData = async () => {
        ...
        await Promise.all([
          handleGetRequisition(Number(requisition.id)),
          handleGetLines(Number(requisition.id)),
        ]);
        setLoading(false);
      };
      loadData();
  ...
  const handleAddItem = (item: any) => {
    console.log("items agregados", requisitionItems);
              ...
              {type ? (
                <div className="flex items-center gap-3 p-3 rounded-xl bg-blue-50 border border-blue-100">
                  <div className="w-8 h-8 rounded-lg bg-blue-100 flex items-center justify-center flex-shrink-0">
                    {/* ícono según el tipo */}
                    {React.createElement(
                      REQUISITION_TYPE_CONFIG[type]?.icon ??
                        MdOutlineInventory2,
                      {
                        size: 16,
                    ...
                    <FormRadioGroup
                      ...
                      options={
                        MOVEMENT_REASON_OPTIONS[
                          form.movement as MovementType
                        ]?.map((opt) => ({
                          label:
                            MOVEMENT_REASON_OPTIONS[
                              form.movement as MovementType
                            ]?.find((o) => o.reason === opt.reason)?.label ??
                            opt.label,
                          value: opt.reason,
                        })) || []
                      }
